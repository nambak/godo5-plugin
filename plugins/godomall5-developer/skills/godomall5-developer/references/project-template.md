...
- [ ] SQL에 바인드 쿼리(`bind_param_push()`)를 사용하는가?
- [ ] 커스텀 테이블에 `dpx_` 접두사를 사용하는가? (`es_`/`zz_` 금지)
- [ ] `\Logger::channel('userLog')->debug()`로 로깅하는가?
...
- 500 에러/예상치 못한 동작 → **코드 로직과 클래스 계층 구조 먼저 확인**
- 구체적 증거 없이 OPcache 초기화, DB 트랜잭션 격리 수준 등으로 추측하지 말 것
- 순서: 코드 로직 → 클래스 상속 체인 → DB 쿼리 → 프레임워크 설정
